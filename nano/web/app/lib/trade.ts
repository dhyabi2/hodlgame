...
export async function execBuy(
  ...
  const raw = BigInt(Math.floor(xnoAmount * 1e30)) || 0n;
...
export async function execSell(
  ...
  const raw = BigInt(Math.floor(tokenAmount * 10 ** token.decimals)) || 0n;
...
export async function execTransfer(
  ...
  const raw = BigInt(Math.floor(tokenAmount * 10 ** token.decimals)) || 0n;
  if (!to.startsWith("nano_")) throw new Error("recipient must be a nano_ address");
